...
public class _ServerUtil {
    ...
    public static String getServerId() {
        _ServerUtil sd = _instance;
        if (sd._serverId == null) {
            if (_ServerUtil.isGeronimo()) {
                sd._serverId = "geronimo";
            } else if (_ServerUtil.isJBoss()) {
                sd._serverId = "jboss";
            } else if (_ServerUtil.isJOnAS()) {
                sd._serverId = "jonas";
            } else if (_ServerUtil.isOC4J()) {
                sd._serverId = "oc4j";
            } else if (_ServerUtil.isOrion()) {
                sd._serverId = "orion";
            } else if (_ServerUtil.isResin()) {
                sd._serverId = "resin";
            } else if (_ServerUtil.isWebLogic()) {
                sd._serverId = "weblogic";
            } else if (_ServerUtil.isWebSphere()) {
                sd._serverId = "websphere";
            }
            if (_ServerUtil.isJetty()) {
                if (sd._serverId == null) {
                    sd._serverId = "jetty";
                } else {
                    sd._serverId += "-jetty";
                }
            } else if (_ServerUtil.isTomcat()) {
                if (sd._serverId == null) {
                    sd._serverId = "tomcat";
                } else {
                    sd._serverId += "-tomcat";
    ...
    private static _ServerUtil _instance = new _ServerUtil();
