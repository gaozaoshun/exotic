...
layui.use(['layer','jquery','form','icheck', 'laypage','laydate'],function () {
    ...
    var pagesUrl = "../../comments/pages";
    var queryUrl = "../../comments/get";
    var userUrl = "../../user/get";
    var drUrl = "../../comments/dr";
    var delUrl = "../../comments/delete/real";

    var dataForm = {page:1,rows:10,field:3,order:1};//查询参数
    managerComment.initUI($,layer,laypage,form,laydate,pagesUrl,queryUrl,userUrl,dataForm,drUrl,delUrl);//初始化界面
});
var managerComment = {
    initUI:function ($,layer,laypage,form,laydate,pagesUrl,queryUrl,userUrl,dataForm,drUrl,delUrl) {
        $(function () {
            managerComment.initData($,laypage,form,laydate,pagesUrl,queryUrl,dataForm,drUrl,delUrl);//初始化数据
            managerComment.regSearch($,laypage,form,laydate,pagesUrl,queryUrl,drUrl,delUrl,userUrl);//注册搜索按钮
        })
    },
    regSearch:function ($,laypage,form,laydate,pagesUrl,queryUrl,drUrl,delUrl,userUrl) {
            managerComment.showHighQ($);//注册高级搜索栏显示事件
            managerComment.regSelectBox($,form,userUrl);//加载用户---高级搜索栏
            $('#titleQ').on('click',function (e) {//监听普通查询事件
                ...
                managerComment.initData($,laypage,form,laydate,pagesUrl,queryUrl,dataForm,drUrl,delUrl);
            })
            //TODO 监听高级查询事件
                        ...
                        if(res.code==200){
                            ...
                            $.each(res.data,function (index,item) {
                                tr += '<tr>'+
                                    ...
                                    '<td>'+item.id+'</td>'+
                                    '<td><a href="#">'+item.blog.title+'</a></td>'+
                                    '<td>'+item.content+'</td>'+
                           ...
                           $(tr).appendTo('#tbody').slideDown();
                            //TODO 按钮注册
                            managerComment.regCheckBox($);//注册icheck组件
                            managerComment.regRepBtn($,form,drUrl);//注册撤销按钮点击事件
                            managerComment.regRecBtn($,form,drUrl);//注册恢复按钮点击事件
                            managerComment.regDelBtn($,form,delUrl);//注册删除按钮点击事件
                        }else{
                            parent.layer.msg(res.msg,{icon:2,shade:0.5});
                        }
                    },
    ...
    regDelBtn:function ($,form,delUrl) {
        ...
        $($opera).unbind('click').bind('click',function (e) {
            ...
            managerComment.commonAction($,delUrl,{ids:[dataId]},[$tr]);//删除事件
        }),
        //页面删除按钮注册
        $('#delBtn').unbind('click').bind('click',function (e) {
            ...
            if(checkedList.length<1) {
                ...
                managerComment.commonAction($,delUrl,{ids:checkedList},trs);//删除事件
            }
        })
    },
    regRecBtn:function ($,form,drUrl) {
        ...
        $('#recBtn').unbind('click').bind('click',function (e) {
            ...
            if(checkedList.length<1) {
                ...
                managerComment.commonAction($,drUrl,{ids:checkedList,dr:0},trs);//恢复事件
            }
        })
    },
    regRepBtn:function ($,form,drUrl) {
        ...
        $($opera).unbind('click').bind('click',function (e) {
            ...
            if ($(this).text().indexOf("撤销")!=-1){
                managerComment.commonAction($,drUrl,{ids:[dataId],dr:1},[$tr]);//撤销事件
            }
            if ($(this).text().indexOf("恢复")!=-1){
                managerComment.commonAction($,drUrl,{ids:[dataId],dr:0},[$tr]);//恢复事件
            }
        }),
        //页面撤销按钮注册
        $('#repBtn').unbind('click').bind('click',function (e) {
            ...
            if(checkedList.length<1) {
                ...
                managerComment.commonAction($,drUrl,{ids:checkedList,dr:1},trs);//撤销事件
            }
        })
    },
    ...
    commonAction:function ($, url, params,trs) {
        ...
        setTimeout(function () {
            $.ajax({
                ...
                success:function (res,state) {
                    if(res.code==200){
                        managerComment.flushTable($,url,params,trs); //刷新表格
                    ...
                    if(res.code==200){
                        laypage({
                            ...
                            jump: function(obj, first) {
                                ...
                                managerComment.jumpPage($,laydate,form,obj,first,queryUrl,dataForm,drUrl,delUrl);
                            }
                        });
                        parent.layer.close(index);
                    }else{
