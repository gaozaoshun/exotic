...
layui.use(['layer','jquery','form','icheck', 'laypage','laydate'],function () {
    ...
    var pagesUrl = "../../sys/user/pages";
    var queryUrl = "../../sys/user/get";
    var addUrl = "../../sys/user/add";
    var userUrl = "../../sys/user/get";
    var drUrl = "../../sys/user/dr/v2";
    var delUrl = "../../sys/user/delete";
    var recomUrl = "../../sys/user/modify";
    var giveUrl = "../../sys/user/give";
    var dataForm = {page:1,rows:10,field:3,order:1};//查询参数
    managerSysUser.initUI($,layer,laypage,form,laydate,pagesUrl,queryUrl,userUrl,dataForm,drUrl,delUrl,addUrl,recomUrl,giveUrl);//初始化界面
});
var managerSysUser = {
    initUI:function ($,layer,laypage,form,laydate,pagesUrl,queryUrl,userUrl,dataForm,drUrl,delUrl,addUrl,recomUrl,giveUrl) {
        $(function () {
            managerSysUser.initData($,laypage,form,laydate,pagesUrl,queryUrl,dataForm,drUrl,delUrl,addUrl,recomUrl,giveUrl);//初始化数据
            managerSysUser.regSearch($,laypage,form,laydate,pagesUrl,queryUrl,drUrl,delUrl,userUrl,addUrl,recomUrl,giveUrl);//注册搜索按钮
        })
    },
    regSearch:function ($,laypage,form,laydate,pagesUrl,queryUrl,drUrl,delUrl,userUrl,addUrl,giveUrl) {
            $('#titleQ').on('click',function (e) {//监听普通查询事件
                ...
                managerSysUser.initData($,laypage,form,laydate,pagesUrl,queryUrl,dataForm,drUrl,delUrl,addUrl,giveUrl);
            })
    },
                ...
                $.ajax({
                    ...
                    success:function (res,state) {
                        ...
                        if(res.code==200){
                            ...
                            var tr = '' ;
                            $.each(res.data,function (index,item) {
                                var minImg = item.headimg;
                                var maxImg = minImg.replace('min','max');
                                tr += '<tr>'+
                                    '<td><input type="checkbox" lay-filter="check" data-id="'+item.id+'"></td>'+
                                    '<td>'+item.id+'</td>'+
                                    '<td><a href="javascript:;"><img src="'+minImg+'" layer-src="'+maxImg+'" alt="'+item.nickname+'" style="width: 50px;height: 50px;border-radius: 50%;border: 10px ridge #F4E9E0;"></a></td>'+
                                    '<td>'+item.username+'</td>'+
                                    '<td>'+item.password+'</td>'+
                                    '<td>'+item.nickname+'</td>'+
                                    '<td>'+item.state+'</td>'+
                           ...
                           $(tr).appendTo('#tbody');
                            ...
                            //背景图预览
                            managerSysUser.preview();
                            //TODO 按钮注册
                            managerSysUser.regCheckBox($);//注册icheck组件
                            managerSysUser.regAddBtn($,form,addUrl);//添加
                            managerSysUser.regEditBtn($,form,recomUrl);//编辑
                            managerSysUser.regSaveBtn($,form,drUrl);//更新
                            managerSysUser.regDelBtn($,form,delUrl);//删除
                            managerSysUser.regGiveBtn($,form,giveUrl);//授予角色
                        }else{
                            parent.layer.msg(res.msg,{icon:2,shade:0.5});
                        }
                    },
        ...
        $('#giveBtn').off('click').on('click',function (e) {
            ...
            if(checkedList.length!=1) {
                ...
                parent.layer.open({
                    ...
                    title: '授予角色',
                    area: ['450px', '420px'],
                    content: 'menus/giveRoles.html?id=' + checkedList[0],
                    shade: 0.5,
                    end: function () {
                        //刷新表格
    ...
    regSaveBtn:function ($,form,drUrl) {
        $('#saveBtn').off('click').on('click',function (e) {
            ...
            setTimeout(function () {
                $.ajax({
                    url:drUrl,
                    type:'post',
                    data:JSON.stringify({users:params}),
        ...
        $('#editBtn').off('click').on('click',function (e) {
            ...
            if(checkedList.length!=1) {
                ...
                parent.layer.open({
                    ...
                    title:'编辑当前用户',
                    area:['450px','420px'],
                    content:'menus/modifySysUser.html?id='+checkedList[0],
                    shade:0.5,
                    end:function () {
                        //刷新表格
                        window.location.reload();
    ...
    regDelBtn:function ($,form,delUrl) {
        ...
        $($opera).unbind('click').bind('click',function (e) {
            ...
            managerSysUser.commonAction($,delUrl,{ids:[dataId]},[$tr]);//删除事件
        }),
        //页面删除按钮注册
        $('#delBtn').unbind('click').bind('click',function (e) {
            ...
            if(checkedList.length<1) {
                ...
                managerSysUser.commonAction($,delUrl,{ids:checkedList},trs);//删除事件
            }
        })
    },
    ...
    commonAction:function ($, url, params,trs) {
        ...
        setTimeout(function () {
            $.ajax({
                ...
                success:function (res,state) {
                    if(res.code==200){
                        managerSysUser.flushTable($,url,params,trs); //刷新表格
                    ...
                    if(res.code==200){
                        laypage({
                            ...
                            jump: function(obj, first) {
                                ...
                                managerSysUser.jumpPage($,laydate,form,obj,first,queryUrl,dataForm,drUrl,delUrl,addUrl,recomUrl,giveUrl);
                            }
                        });
                        parent.layer.close(index);
                    }else{
