...
layui.use(['layer','jquery','form','laydate','tree'],function () {
    ...
    var addMenuUrl = "../../sys/autho/add";
    addSysAutho.initUI($,form,laydate,menuUrl,addMenuUrl);//初始化界面
});
var addSysAutho = {
    initUI:function ($,form,laydate,menuUrl,addMenuUrl) {
        $(function () {
            addSysAutho.loadMenuIcon($,form);//加载菜单图标
            addSysAutho.loadMenus($,form,menuUrl);//加载父菜单
            addSysAutho.regSwitch($,form);//监听启用开关
            addSysAutho.regSaveBtn($,form,addMenuUrl);//监听保存按钮
        })
    },
    ...
    loadMenus:function ($,form,menuUrl) {
        ...
        setTimeout(function () {
            $.ajax({
                ...
                success:function (res,state) {
                    if(res.code==200){
                      addSysAutho.appendMenus($,form,res.data);
                    }else{
                        layer.msg(res.msg,{icon:2,shade:0.5});
                    }
                    layer.close(index);
